...
def process_enrichment_data(enrichment_paths, truth_paths, layers=None):
    ...
    for i, (enrich_path, truth_path) in enumerate(zip(enrichment_paths, truth_paths)):
        ...
        dfp = df.iloc[:, [1, 3]]
        dfp['p_value'] = 10 ** (-dfp['EnrichmentScore'])
...
def calculate_correlation_matrix(results_dict):
    """
    Calculate correlation coefficient matrix for p-values across multiple result DataFrames
    
    Parameters:
        results_dict (dict): Dictionary containing result DataFrames
    
    Returns:
        pd.DataFrame: Correlation coefficient matrix
    """
    # Merge all results
    combined_df = None
        ...
        for j, col2 in enumerate(p_value_cols):
            if i <= j:
                ...
                if valid_mask.sum() > 1:  # At least 2 valid values required
                    corr, p_val = pearsonr(combined_df.loc[valid_mask, col1], 
    ...
    simplified_index = [f'df{i+1}' for i in range(n)]
...
def main_analysis():
    ...
    print("\nPairwise Pearson correlation matrix for Combined_p_value:")
    print(corr_matrix)
